...
if "%USE_CUDA%"=="1" (
  ...
  curl -L https://github.com/peterjc123/randomtemp-rust/releases/download/v0.4/randomtemp.exe --output %TMP_DIR_WIN%\bin\randomtemp.exe
  if errorlevel 1 goto fail
  if not errorlevel 0 goto fail
  echo @"%TMP_DIR_WIN%\bin\randomtemp.exe" "%TMP_DIR_WIN%\bin\sccache.exe" "%CUDA_PATH%\bin\nvcc.exe" %%* > "%TMP_DIR_WIN%\bin\nvcc.bat"
  type "%TMP_DIR_WIN%\bin\nvcc.bat"
  ...
  set CMAKE_CUDA_COMPILER_LAUNCHER=%TMP_DIR_WIN%\bin\randomtemp.exe;%TMP_DIR_WIN%\bin\sccache.exe
)
...
if not errorlevel 0 goto fail
sccache --show-stats
python -c "import sys, glob, subprocess; w = glob.glob('dist/*.whl'); sys.exit('ERROR: no wheel found in dist/ after build') if not w else sys.exit(subprocess.run([sys.executable, '-m', 'pip', 'install', '--no-index', '--no-deps', w[0]]).returncode)"
if errorlevel 1 goto fail
if not errorlevel 0 goto fail

copy /Y "dist\*.whl" "%PYTORCH_FINAL_PACKAGE_DIR%"
if errorlevel 1 (
  echo ERROR: failed to copy dist\*.whl to "%PYTORCH_FINAL_PACKAGE_DIR%". Ensure the destination exists and is writable.
  goto fail
)
