...
<#
.SYNOPSIS
  GitHub Actions entrypoint for the WoA torchvision extension wheel build.
...
.DESCRIPTION
  ...
  pytorch/vision, wires vcpkg/codecs, pip-wheels torchvision, and delvewheel-
  repairs it into cuda_embed_dlls/), then copies the produced torchvision wheel
  into a flat -OutputDir.
...
$rc = Resolve-BuildFlowExitCode (Invoke-TorchvisionWindowsBuildFlow)
if ($rc -ne 0) {
    Write-Host "::error title=woa torchvision build::torchvision build-flow returned $rc"
    exit $rc
}

# Collect the torchvision wheel (built into <WHEEL_OUT_ROOT>\cuda_embed_dlls) into OutputDir.
New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
$marker = Join-Path (Join-Path $env:CI_PROJECT_DIR 'logs') 'WHEEL_OUT_ROOT'
if (-not (Test-Path -LiteralPath $marker)) {
    Write-Host "::error title=woa torchvision build::WHEEL_OUT_ROOT marker not found at $marker"
    exit 1
}
$embedDir = Join-Path ((Get-Content -LiteralPath $marker -Raw).Trim()) 'cuda_embed_dlls'
$whl = Get-ChildItem -LiteralPath $embedDir -Filter 'torchvision-*.whl' -File -ErrorAction SilentlyContinue |
    Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $whl) {
    Write-Host "::error title=woa torchvision build::no torchvision wheel under $embedDir"
    exit 1
}
Copy-Item -LiteralPath $whl.FullName -Destination $OutputDir -Force
Write-Host "Staged torchvision wheel -> $OutputDir\$($whl.Name)"
exit 0
