...
# $BUILD_ENVIRONMENT is expected to be set by the caller / runner image.
set -ex -o pipefail

export TMP_DIR="${PWD}/build/win_tmp"
TMP_DIR_WIN=$(cygpath -w "${TMP_DIR}")
export TMP_DIR_WIN
# Create the scratch dir up front: install_magma.bat curls into TMP_DIR (curl
# won't create missing parents) and install_sccache.bat / randomtemp write into
# TMP_DIR/bin. Upstream relies on its setup-win action for this; we don't run it.
mkdir -p "${TMP_DIR}/bin"
...
# build_pytorch.bat sits alongside this script and locates its own helpers
# (install_*.bat, activate_miniconda3.bat) via %~dp0.
"$SCRIPT_PARENT_DIR"/build_pytorch.bat
