...
@{
    Variables = @{
        ...
        'PYTORCH_WIN_BUILD_SLEEF_DISABLE_SVE'           = @{ Type = 'Bool'; Domain = 'Build'; DefaultKey = 'SleefDisableSve';         Required = $false; Secret = $false; Description = 'Exports SLEEF_DISABLE_SVE for the PyTorch build. CMake reads the matching -DSLEEF_DISABLE_SVE cache entry from CMAKE_ARGS, not this env var, so the two must be flipped together. Independent of the -A platform flag: switching to the VS generator keeps -DSLEEF_DISABLE_SVE=ON alongside -A ARM64.' }
        ...
        'PYTORCH_WIN_BUILD_CMAKE_GENERATOR'             = @{ Type = 'String'; Domain = 'Build'; DefaultKey = 'CMakeGenerator';        Required = $false; Secret = $false; Description = 'CMAKE_GENERATOR selecting the CMake build system (default Ninja). To revert, set a Visual Studio generator and include -A ARM64 alongside -DSLEEF_DISABLE_SVE=ON in PYTORCH_WIN_BUILD_CMAKE_ARGS.' }
        'PYTORCH_WIN_BUILD_CMAKE_ARGS'                  = @{ Type = 'String'; Domain = 'Build'; DefaultKey = 'CMakeArgs';             Required = $false; Secret = $false; Description = 'CMAKE_ARGS forwarded to CMake. The default disables SLEEF SVE targets; it omits the Ninja-incompatible -A platform flag because the ARM64 target comes from the imported vcvarsarm64 environment instead.' }
